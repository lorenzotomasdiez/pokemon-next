...
const PokemonPageName: NextPage<Props> = ({pokemon}) => {
  const [exist, setExist] = useState(localFavorites.existInFavorites(pokemon.id))
        ...
        <Grid xs={12} sm={8}>
          <Card>
            <Card.Header css={{display: 'flex', justifyContent: 'space-between'}}>
              ...
              <Button color='gradient' ghost={!exist} onClick={onToggleFavorite}>
                {
                  exist ? 'Favorito' : 'Guardar en favoritos'
                }
              </Button>
            </Card.Header>
            <Card.Body>
...
export const getStaticPaths: GetStaticPaths = async (ctx) => {
  ...
  const result = data.results.map((e) => e.name)
  return {
